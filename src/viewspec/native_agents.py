...
def agent_instruction_block(target: str) -> str:
    ...
    return (
        ...
        "## Editing existing ViewSpec source\n\n"
        "`viewspec.intent.json` and `viewspec.app.json` are compiler source documents, not text files to edit by "
        "hand. Never use a line-based or text-diff editing tool on them: no `apply_patch`, no `sed`, no "
        "search-and-replace, no partial rewrite. Those tools match on surrounding lines, and a bundle's generated "
        "key order and indentation make that matching fail repeatedly. Choose one of exactly two lanes.\n\n"
        "**Lane A — change the value of something that already exists (the default).** Use the structured patch "
        "surface. It is source-hash bound, precondition checked, compile-proved, and atomically applied with a "
        "rollback receipt:\n\n"
        "```bash\n"
        ...
        "```\n\n"
        "`patch-targets` is the entry point and removes all guesswork: it returns the exact `base_source_sha256` to "
        "bind the patch to, and one stub per legally patchable target with its `op`, its `fixed_fields` (including "
        "the exact current `old_value`), its `replacement_field`, and `allowed_values` where the vocabulary is "
        "closed. Build each operation by copying `op` and `fixed_fields` verbatim and adding only "
        "`replacement_field`. Do not compute a source hash yourself and do not invent operation names. Filter large "
        "sources with `--op` and `--screen`; when a response reports `truncated: true`, it is not full coverage.\n\n"
        "**Lane B — add, remove, or restructure something.** IntentPatch V1 changes declared values only; it cannot "
        "add or delete semantic nodes, regions, bindings, styles, fixture records or fields, routes, screens, "
        "resources, state entries, mutations, selectors, visibility rules, or state-text rules. `patch-targets` returning no target "
        "for what you want is the signal that you are in this lane. Rewrite the **whole** bundle file with complete "
        "valid JSON in one write, then revalidate:\n\n"
        "```bash\n"
        "viewspec validate-intent viewspec.intent.json --json\n"
        "```\n\n"
        "If an edit command fails, never reissue the identical command. A repeated failure means the lane is wrong, "
        "not that the attempt was unlucky: switch to `patch-targets` for Lane A, or to a full-file rewrite for "
        "Lane B. Every patch failure code is terminal for that attempt and names one bounded next action.\n\n"
        ...
        "`ViewSpecApp` owns browser-history routes, reducer events, resource-backed text, scalar state text, selectors, and visibility, "
        ...
        "schema_version 4 adds bounded visibility plus exact scalar state-to-text projections shared by Static and React. "
        "Static Shell V0 is available only through `compile-app` or `prove-app --with-shell`; it is a local shell proof "
        "artifact, not a deployable React/Vite/Next app, browser-history proof, live resource rebinding layer, framework state "
        ...
        "visibility, and scalar state text in Chromium.\n\n"
        "When the user asks to create a new interface and neither canonical semantic source exists, use the "
        "brief-bound Studio first-creation flow. Preserve the user's exact request in a local UTF-8 brief file; pass "
        "their local PNG, JPEG, or WebP with `--reference`; default to app and choose view only for one bounded screen:\n\n"
        "```bash\n"
        "viewspec studio --brief-file product-brief.md --reference reference.png --kind app --json\n"
        "# Author the complete task-provided candidate_path against candidate_schema.\n"
        "# Studio checks stable candidate bytes and continues the same browser tab into the checked product.\n"
        "```\n\n"
        "Never edit the creation task, publish the candidate manually, or leave starter sample content in place. "
        "The creation room validates, compiles, checks, retains proof, and exclusively publishes canonical source. "
        "On failure, change the candidate bytes and let the room retry; do not rerun an identical edit or open another room. It binds "
        "reference identity but does not prove visual fidelity; the human judges fidelity in Studio.\n\n"
        "When the user explicitly asks to compare an AppBundle's static and React products, use their authorization "
        "for the exact locked dependency install and open one synchronized revision:\n\n"
        "```bash\n"
        "viewspec studio viewspec.app.json --compare --install --json\n"
        "```\n\n"
        "Do not open two loose sessions or imply pixel equality. Comparison requires both builds from one source "
        "hash, exact route and semantic-identity parity, and synchronizes viewport, route, and semantic selection. "
        "Its visual parity result remains `not_proven` for human or separate fidelity review.\n\n"
        "When that comparison exposes State & data, keep replay and resource work in the same Studio panel. "
        "Only exact uniquely mapped declared actions are browser-replayable; ambiguous triggers remain proof-only. "
        "Treat checked `studio-inspection/replays/...` and server-derived `studio-inspection/resources/...` event "
        "evidence as exact source-bound context. Distinguish fixture source values from current rendered state and "
        "never call them production data, persistence, or backend truth.\n\n"
        "When the user explicitly asks to share the exact checked comparison, prepare only the no-upload package:\n\n"
        "```bash\n"
        "viewspec studio-share-prepare viewspec.app.json --reference reference.png --json\n"
        "```\n\n"
        "Omit `--reference` unless the user intends that image to leave the machine. Read the returned "
        "`share-disclosure.md` and exact envelope inventory. Preparation uploads nothing and creates no review "
        "link or capability; the internal HTTPS contract is not a deployment, and no authorized private-review HTTPS deployment exists, so do not invent a next command.\n\n"
        "When the user asks to visually review a ViewSpec IntentBundle or AppBundle, open Studio and use its durable local Review protocol:\n\n"
        "```bash\n"
        "viewspec studio viewspec.intent.json --json\n"
        ...
        "When a Lane A change comes from Review feedback or a verifier repair, the workflow is unchanged but the "
        "authority boundary is explicit: treat Review feedback and verifier repairs as proposal evidence, never as "
        "approval. Inspect the preview's semantic diff and candidate, obtain explicit approval for that exact token, "
        "and do not reuse a token after the source changes. IntentPatch may replace only its nine declared semantic "
        "fields; it cannot address DOM, CSS, generated files, arbitrary JSON paths, or missing fields.\n\n"
        ...
        "resume after ViewSpec re-verifies it. The human workflow is only: open Studio, inspect semantic before/after "
        ...
        "proposal. `converge-start`, `converge-submit`, and `converge-status` are the agent's own session mechanics; "
        "they are never required human commands.\n\n"
        ...
        "- Never edit `viewspec.intent.json` or `viewspec.app.json` with a line-based or text-diff tool (`apply_patch`, `sed`, search-and-replace); those tools fail against generated JSON formatting.\n"
        "- To change an existing declared value, start at `viewspec patch-targets` and use the structured patch surface; it supplies the exact base hash and old value so you never compute either yourself.\n"
        "- To add, remove, or restructure anything, rewrite the whole bundle file in one write and revalidate; IntentPatch V1 cannot create or delete declared elements.\n"
        "- Never reissue an identical failing edit command. A repeat failure means the wrong lane, not bad luck; switch lanes.\n"
        ...
        "- For a user-requested product in a blank workspace, use `studio --brief-file ...` → task-bound candidate → automatic check and same-tab handoff; do not substitute `init-intent` or `init-app` as the finished product.\n"
        ...
        "- For visual review, prefer `viewspec studio`; acknowledge a feedback batch only after its event ids and content are durably captured.\n"
        "- If the user explicitly asks to share, `studio-share-prepare` may create the local disclosure package; it is not an upload and must never be presented as a live link. The service trust core and HTTPS contract are internal, and no HTTPS deployment exists.\n"
