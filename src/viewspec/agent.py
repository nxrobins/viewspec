...
The default html-tailwind-app Static Shell V0 remains a local proof artifact, not a deployable framework app: it performs bounded visibility toggling only and does not prove runtime data or text rebinding. The additive react-tailwind-app target emits a complete Vite/React/Tailwind package and generated Playwright proof. Its exact-artifact verifier checks every generated hash, runs the Vite production build, and exercises routes, browser history, unknown-route fallback, declared mutations, live resource-field rebinding, selector replay, and visibility in Chromium.
