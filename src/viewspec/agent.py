...
If the host explicitly asks for a bounded IntentPatch instead of a full regenerated bundle, follow intent-patch.schema.json exactly: bind it to the exact source SHA-256, use only the nine declared stable-id operations, and include exact old values. A patch is a proposal, not approval. The host must run patch-preview, inspect the candidate and semantic diff, obtain approval for that exact approval token, and only then call patch-apply; never infer approval from Review feedback or verifier output, never rebase a stale patch, and never target generated files, DOM, CSS, arbitrary JSON paths, or missing fields.
