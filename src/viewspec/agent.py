...
For a multi-screen internal tool, emit AppBundle JSON as viewspec.app.json instead of inventing a router or app scaffold. Use schema_version 1 for unbound fixture context, or schema_version 2 with resource_binding "fixture_readonly_v0" and per-screen resource_views when the proof must verify exact fixture scalar visibility inside declared target motifs; generate that starter with viewspec init-app --resource-binding fixture-readonly-v0 --out viewspec.app.json. Validate with viewspec validate-app viewspec.app.json --json, review changes with viewspec diff-app old.app.json new.app.json --json, compile a local Static Shell V0 artifact with viewspec compile-app viewspec.app.json --out app-dist --target html-tailwind-app --json, and prove with viewspec prove-app --app viewspec.app.json --out .viewspec-app-proof --with-shell --json. This is an app contract, static shell artifact, per-screen source-artifact proof, and optional read-only fixture visibility proof only: no dynamic routes, browser history proof, runtime data binding, deployable React/Vite/Next app, backend, state, mutations, or hosted extended compiler behavior.
