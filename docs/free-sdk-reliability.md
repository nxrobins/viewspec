...
Public pricing, version, hosted-call, API, package, proof-scope, proof identity, and agent asset contract facts live in `demos/public-facts.json`; the static smoke test fails with `PUBLIC_FACTS_DRIFT` if README, landing, LLM, OpenAPI, agent asset, or version metadata disagree with it.
