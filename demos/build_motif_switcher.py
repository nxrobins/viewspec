  ...
  <main class="mx-auto flex min-h-screen max-w-6xl flex-col px-4 py-8 sm:px-6 lg:px-8">
    <header class="mb-8 grid gap-6 border-b border-white/10 pb-8 lg:grid-cols-[1fr_auto] lg:items-end">
      <div class="max-w-3xl">
        ...
        <h1 class="text-4xl font-black leading-tight text-white sm:text-5xl">Same Data, Three Motifs</h1>
        <p class="mt-4 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
          One semantic team roster compiles into three visual structures. The substrate and bindings stay fixed; only the motif hint changes.
        </p>
      </div>
    ...
    <section class="mb-6 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
      <div>
        <h2 id="motif-title" class="text-xl font-bold text-white">Table</h2>
        <p id="motif-description" class="mt-1 text-sm leading-6 text-slate-400">Dense rows for scanning operational data.</p>
      </div>
...
{variants_json}
  ...
  <script>
    ...
    function activateMotif(kind) {{
      ...
      title.textContent = motifData[kind].label;
      description.textContent = motifData[kind].description;
    }}
