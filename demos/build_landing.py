...
(function(){
  ...
  var METRICS=[
    ...
    {ml:"runtime LLM",    mv:"none",   seed:1.1},
  ...
  function legacyCopy(text){
    ...
    var copied=false; try{ copied=document.execCommand("copy"); }finally{ input.remove(); }
  ...
  function copyText(text){
    ...
    try{ legacyCopy(text); return Promise.resolve(); }catch(error){ return Promise.reject(error); }
  }
  document.querySelectorAll("[data-copy-text]").forEach(function(button){
    var original=button.textContent;
    button.addEventListener("click",function(){
      button.disabled=true; button.setAttribute("data-copy-state","pending");
      copyText(button.getAttribute("data-copy-text")||"").then(function(){
        button.textContent="copied"; button.setAttribute("data-copy-state","success");
      }).catch(function(){
        button.textContent="copy failed"; button.setAttribute("data-copy-state","error");
      }).then(function(){ setTimeout(function(){ button.textContent=original; button.disabled=false; button.removeAttribute("data-copy-state"); },1400); });
    });
  });
