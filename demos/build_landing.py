...
PAGE_CSS = r"""
  *,*::before,*::after{ box-sizing:border-box; }
  ...
  .mgrid{ display:grid; grid-template-columns:repeat(var(--cols),minmax(0,1fr)); gap:14px; transition:grid-template-columns .35s ease; }
  .metric{ background:var(--panel-2); border:1px solid var(--line); border-radius:var(--rad); padding:calc(var(--u)*2.1); }
  .metric .ml{ font-family:var(--mono); font-size:10.5px; letter-spacing:.12em; text-transform:uppercase; color:var(--muted); }
  .metric .mv{ font-family:var(--mono); font-weight:var(--hw); font-size:clamp(20px,calc(1.4vw*var(--disp)+11px),29px); margin-top:8px; font-variant-numeric:tabular-nums; letter-spacing:-.01em; }
...
(function(){
  ...
  var PROFILES={
    ...
    data_dense:{density:"compact",emphasis:"medium",columns:4},
  ...
  function spark(seed,el,fill){
    ...
    el.innerHTML='<path d="'+d+'L120 34 L0 34 Z" fill="'+(fill||"rgba(245,178,63,.10)")+'"/><path d="'+d+'" fill="none" stroke="var(--amber)" stroke-width="1.5"/><circle cx="120" cy="'+last+'" r="2.2" fill="var(--amber-2)"/>';
  }
